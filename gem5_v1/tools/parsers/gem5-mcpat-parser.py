...
import re
######################################################################################
########Execute parser with following command:########################################
########python parser.py statsInputPath xmlOutputPath parameterInputPath##############
######################################################################################
statsInPath = str(sys.argv[1])
xmlOutPath = str(sys.argv[2])
paramPath = str(sys.argv[3])

shutil.copy2('/home/subodha/garnet-gem5/tools/parsers/Xeon.xml', xmlOutPath) # complete target filename given, (golden xml path, output new xml path)
#shutil.copy2('/home/vnikhil/Parser/Xeon.xml', '/home/vnikhil/Parser/Xeon_new2.xml') # complete target filename given, (golden xml path, output new xml path)
tree = ET.parse(xmlOutPath)
#tree = ET.parse('/home/vnikhil/Parser/Xeon_new.xml')
root = tree.getroot()
...
#####################################################	
#Parameter file open
pfile = open(paramPath, 'r')
#pfile = open('/home/vnikhil/Parser/parameters.txt', 'r')
pfiletext = pfile.read()
#Stats file open
sfile = open(statsInPath, 'r')
#sfile = open('/home/vnikhil/Parser/stats_new.txt', 'r')
...
#find FLOAT value for string in stats file
def findValueStatsFloat(str):
	regx = re.escape(str) + ".*$"
	items=re.findall((regx),sfiletext,re.MULTILINE)
	#val =  ([int(s) for s in items[0].split() if s.isdigit()][0])
	nums=0
...
total_cycles = findSumStats("numCycles")/100
idle_cycles = findSumStats("num_idle_cycles")/100
busy_cycles = findSumStats("num_busy_cycles")/100
...
NoC0_total_accesses = int(round(findValueStats("system.ruby.network.flits_injected::total") * findValueStatsFloat("system.ruby.network.average_hops")))
#print NoC0_total_accesses
...
outfile = open(xmlOutPath,'w')
#outfile = open(r'/home/vnikhil/Parser/Xeon_new2.xml','w')
